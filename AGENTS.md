...
AIGC Prompt 工具站（`aigcprompt-site`），为"入行365"生态提供 AIGC 提示词库、风格技能包生成与管理功能。

公开页面、Prompt 说明、技能包说明、产品文案和用户引导必须优先使用自然、具体、像真人说话的表达；避免 PRD 腔、技术黑话、过度抽象、堆概念、强营销口号和生硬的防御性声明。不能为了更口语而删掉核心路径、CTA、测试合同、合规边界或产品交付边界。

## 2. 技术栈

| 层 | 技术 |
|---|---|
| 框架 | Vite 7 + React 19（ESM） |
| 后端/数据 | Supabase |
| 支付 | Stripe |
| UI 组件 | Lucide React |
| 语言 | JavaScript（ESM） |
...
```bash
npm run dev          # 本地开发（含预构建：生成站点数据 + 风格技能包）
npm run build        # 生产构建
npm run preview      # 预览生产构建
npm run generate:site-data    # 生成站点数据
npm run generate:style-skill  # 生成风格技能包
npm run install:skill         # 安装风格技能包
npm run publish:skill:npm     # 发布技能包到 npm
```

## 4. 项目特殊说明

- `scripts/generate-site-data.mjs` 和 `scripts/generate-style-skill.mjs` 在 `dev` / `build` 前自动执行。
- `agents/skills/gpt-image-2-style-library` 目录为可独立发布的 npm 技能包。
- 勿随意修改 `vite.config.*` 等全局构建配置。
